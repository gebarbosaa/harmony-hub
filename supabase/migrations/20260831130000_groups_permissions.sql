...
select p.household_id, p.id, 'OWNER', 'ACTIVE', p.created_at
...
with check (household_id = public.current_household_id() and public.has_household_permission(household_id, 'MANAGE_MEMBERS'));
create policy group_members_update on public.group_members for update to authenticated
using (household_id = public.current_household_id() and public.has_household_permission(household_id, 'MANAGE_MEMBERS'))
with check (household_id = public.current_household_id() and public.has_household_permission(household_id, 'MANAGE_MEMBERS'));
create policy group_members_delete on public.group_members for delete to authenticated
using (household_id = public.current_household_id() and public.has_household_permission(household_id, 'MANAGE_MEMBERS'));

-- Existing onboarding RPCs are upgraded so every group has a membership record.
...
begin
  if uid is null then raise exception 'Não autenticado'; end if;
  select id into target from public.households where invite_code = upper(trim(invite));
  ...
  values (uid, target, upper(coalesce(nullif(trim(my_name), ''), 'USUÁRIO')), upper(substring(coalesce(nullif(trim(my_name), ''), 'US') from 1 for 2)), '#3B9DFF')
  on conflict (id) do update set household_id = target, name = excluded.name, updated_at = now();
  insert into public.group_members(household_id, user_id, role, status)
  values (target, uid, 'MEMBER', 'ACTIVE')
  on conflict (household_id, user_id) do update set status = 'ACTIVE', updated_at = now();
  return target;
end;
$$;
