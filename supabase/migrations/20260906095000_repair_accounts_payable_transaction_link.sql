-- Root fix for Contas a Pagar ledger synchronization.
-- The previous AFTER trigger attempted to mutate NEW.linked_transaction_id, which
-- does not persist the generated transaction id. Persist the link explicitly with
-- a guarded UPDATE after the transaction is inserted.

create unique index if not exists transactions_accounts_payable_source_uq
  on public.transactions (household_id, source_id)
  where source_type = 'ACCOUNTS_PAYABLE' and source_id is not null;
...
begin
  ...
  select t.id into v_tx
   ...
   where t.household_id = new.household_id
     and t.source_type = 'ACCOUNTS_PAYABLE'
  ...
  if v_tx is null then
    insert into public.transactions(
      ...
      paid,payment_method_name,payment_method_id,card_id,account_id,source_type,source_id,
      source_index,source_total,source_period
    ) values (
  ...
  update public.accounts_payable
     set linked_transaction_id = v_tx,
         updated_at = now()
   where id = new.id
     and household_id = new.household_id
     and (linked_transaction_id is distinct from v_tx);
