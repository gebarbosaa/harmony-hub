...
begin
  ...
  if nullif(trim(household_name), '') is null then raise exception 'Informe o nome do grupo.'; end if;
  loop
    code := upper(substring(replace(gen_random_uuid()::text, '-', '') from 1 for 6));
    exit when not exists (select 1 from public.households h where upper(h.invite_code) = code);
  end loop;
  ...
  values (uid, new_id, upper(display_name), upper(substring(display_name from 1 for 2)))
