...
function CreateButton({ title }: { title: string }) {
  function handleClick() { window.dispatchEvent(new CustomEvent("multicap:open-create", { detail: { title } })); }
...
export function PageHeader({ title, subtitle, action }: { title: string; subtitle?: string; action?: ReactNode }) {
  const noDefaultCreate = ["MODO MERCADO", "HÁBITOS", "LISTA DE COMPRAS", "ROTINA DA CASA", "ANOTAÇÕES"].includes(title.trim().toUpperCase());
...
function CreatePanel({ title, children, className, aside }: { title: string; children: ReactNode; className?: string; aside?: ReactNode }) {
  ...
  useEffect(() => { const openHandler = () => setOpen(true); window.addEventListener("multicap:open-create", openHandler); return () => window.removeEventListener("multicap:open-create", openHandler); }, []);
...
export function StatCard({ label, value, delta, icon, tone = "default" }: { label: string; value: string; delta?: string; icon?: ReactNode; tone?: "default" | "primary" | "success" | "danger" | "info" }) {
  ...
  return <div className="group relative overflow-hidden rounded-2xl border border-border/70 bg-card p-4 transition-all hover:-translate-y-0.5 hover:shadow-lg"><div className="pointer-events-none absolute inset-0 bg-gradient-to-br from-primary/[0.06] via-transparent to-transparent opacity-0 transition-opacity group-hover:opacity-100" />{tone === "primary" ? <div className="gradient-soft pointer-events-none absolute inset-0 opacity-60" /> : null}<div className="relative flex items-start justify-between gap-2"><span className="label-caps text-[9px] font-semibold text-muted-foreground">{label}</span><span className={toneRing}>{icon}</span></div><p className="relative mt-3 text-xl font-bold tracking-tight md:text-2xl">{value}</p>{delta ? <p className={cn("relative mt-1 text-xs", toneRing)}>{delta}</p> : null}</div>;
}
