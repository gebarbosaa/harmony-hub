...
function FlowPage() {
  ...
  return <div className="space-y-5">
    ...
    <Panel title="CADASTRO RÁPIDO">
      <div className="mb-3 flex flex-wrap gap-2">{TYPES.map(t => <button key={t} onClick={() => setType(t)} className={cn("label-caps rounded-lg border px-3 py-1.5 text-[10px]", type === t ? "border-primary bg-primary/15 text-primary" : "border-border text-muted-foreground")}>{t}</button>)}</div>
      <div className="grid gap-3 md:grid-cols-4">
        ...
        <button onClick={add} className="gradient-primary label-caps mt-auto rounded-xl px-4 py-2.5 text-[11px] text-primary-foreground md:col-span-2">SALVAR LANÇAMENTO</button>
      </div>
    </Panel>
    <Panel title="LANÇAMENTOS" aside={<div className="flex flex-wrap items-center gap-2"><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar…" className="rounded-lg border border-input bg-background px-2 py-1.5 text-xs" /><select value={filter} onChange={e=>setFilter(e.target.value as typeof filter)} className="label-caps rounded-lg border border-input bg-background px-2 py-1.5 text-[10px]">{["TODOS",...TYPES].map(f=><option key={f}>{f}</option>)}</select></div>}>
      {isLoading ? <p className="py-8 text-center text-sm text-muted-foreground">Carregando lançamentos...</p> : visible.length === 0 ? <p className="py-8 text-center text-sm text-muted-foreground">Nenhum lançamento cadastrado.</p> : <ul className="divide-y divide-border">{visible.map(t=><li key={t.id} className="flex items-center justify-between gap-3 py-3"><div className="min-w-0"><p className="label-caps truncate text-[11px]">{t.description}</p><div className="mt-1 flex flex-wrap items-center gap-2"><Tag>{t.category}</Tag><PersonDot name={t.responsible}/><span className="text-[10px] text-muted-foreground">{t.pay_method}</span></div></div><div className="flex items-center gap-3 text-right"><div><p className={cn("text-sm font-bold", typeColor[t.type])}>{t.type === "RECEITA" ? "+" : "-"}{formatCurrency(Number(t.amount))}</p><button onClick={()=>togglePaid(t)}><Tag tone={t.paid ? "success" : "warning"}>{t.paid ? "PAGO" : "PENDENTE"}</Tag></button></div><button onClick={()=>removeRow(t)} className="text-xs text-danger">Excluir</button></div></li>)}</ul>}
    </Panel>
  </div>;
}
