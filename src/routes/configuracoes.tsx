...
function SettingsPage() {
  ...
  async function saveCard() { const value = Number(limit.replace(",", ".")); if (!cardName.trim() || value <= 0) return toast.error("PREENCHA O CARTÃO E O LIMITE"); const payload = { name: cardName.trim().toUpperCase(), brand: brand.trim().toUpperCase() || null, last4: last4.trim() || null, credit_limit: value, close_day: Number(close), due_day: Number(due) }; try { if (cardEdit) { await cards.update(cardEdit, payload); toast.success("CARTÃO ATUALIZADO"); } else { await cards.insert(payload); toast.success("CARTÃO CADASTRADO"); } clearCardForm(); } catch (error) { toast.error(error instanceof Error ? error.message : "ERRO AO SALVAR CARTÃO"); } }
  ...
  return <div className="space-y-5">
    ...
    <Panel title="FORMAS DE PAGAMENTO"><div className="space-y-3"><div className="grid min-w-0 gap-2 sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto]"><input value={pay} onChange={(event) => setPay(event.target.value)} placeholder="NOVA FORMA" className="min-w-0 w-full rounded-xl border p-3"/><input value={payDesc} onChange={(event) => setPayDesc(event.target.value)} placeholder="DESCRIÇÃO" className="min-w-0 w-full rounded-xl border p-3"/><button onClick={() => void addPayment()} className="gradient-primary w-full min-w-0 shrink-0 rounded-xl px-4 py-3 text-[10px] sm:w-auto">ADICIONAR</button></div>{payments.rows.length === 0 ? <p className="py-6 text-center text-sm text-muted-foreground">NENHUMA FORMA CADASTRADA.</p> : payments.rows.map((item) => <div key={item.id} className="rounded-xl border p-3"><div className="flex items-center justify-between gap-2">{payEdit === item.id ? <div className="grid w-full gap-2 sm:grid-cols-2"><input value={payName} onChange={(event) => setPayName(event.target.value)} className="rounded-lg border p-2"/><input value={payDescEdit} onChange={(event) => setPayDescEdit(event.target.value)} className="rounded-lg border p-2"/><button onClick={() => void editPayment(item.id)} className="text-left text-[10px] text-primary">SALVAR</button></div> : <><div><div className="flex items-center gap-2"><p className="text-xs font-semibold">{item.name}</p>{item.card_id && <span className="rounded-full bg-primary/10 px-2 py-0.5 text-[9px] font-semibold text-primary">CARTÃO</span>}</div>{item.description && <p className="text-[10px] text-muted-foreground">{item.description}</p>}</div><span className="flex gap-2"><button onClick={() => { setPayEdit(item.id); setPayName(item.name); setPayDescEdit(item.description ?? ""); }} className="text-[10px] text-primary">EDITAR</button><button onClick={() => void deletePayment(item.id)} className="text-[10px] text-danger">EXCLUIR</button></span></>}</div></div>)}</div></Panel>
