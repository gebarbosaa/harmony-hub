...
function OnboardingPage() {
  const { user, profile, refreshProfile, setActiveHousehold } = useAuth();
  ...
  async function activateAndEnter(householdId: string) {
    setActiveHousehold(householdId);
    await navigate({ to: "/", replace: true });
  }

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault(); setError(null); if (!name.trim()) return setError("Informe seu nome."); setLoading(true);
    try {
      if (mode === "create") {
        ...
        const { data, error: rpcError } = await supabase.rpc("join_household", { invite: inviteCode.trim().toUpperCase(), my_name: name.trim() });
        if (rpcError) throw rpcError;
        const householdId = typeof data === "string" ? data : data?.household_id ?? data?.id;
        if (!householdId) throw new Error("O código foi aceito, mas o sistema não recebeu a casa de destino.");
        setActiveHousehold(String(householdId));
        await navigate({ to: "/", replace: true });
  ...
  async function enterHub() {
    if (!created?.household_id) return;
    setLoading(true); setError(null);
    try {
      setActiveHousehold(created.household_id);
      const refreshed = await refreshProfile();
      if (!refreshed?.household_id) throw new Error("Não foi possível confirmar a casa ativa.");
      await activateAndEnter(created.household_id);
    } catch (err) { setError(err instanceof Error ? err.message : "Não foi possível entrar no Hub."); setLoading(false); }
  }
  async function copyCode() { if (created?.invite_code) { await navigator.clipboard?.writeText(created.invite_code); toast.success("Código copiado!"); } }
  async function shareCode() { if (!created?.invite_code) return; const text = `Entre na minha casa no Harmony Hub usando o código: ${created.invite_code}`; if (navigator.share) await navigator.share({ text }); else await copyCode(); }

  if (created) return <div className="flex min-h-screen items-center justify-center bg-background px-4"><div className="w-full max-w-sm space-y-5 text-center"><div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-primary/10 text-primary"><CheckCircle2 className="h-7 w-7" /></div><div><h1 className="label-caps text-xl tracking-[0.2em]">CASA CRIADA!</h1><p className="mt-2 text-sm text-muted-foreground">Compartilhe este código para convidar pessoas para sua casa.</p></div><div className="rounded-2xl border border-primary/30 bg-primary/5 p-5"><p className="label-caps text-[10px] text-muted-foreground">CÓDIGO DE CONVITE</p><code className="mt-3 block text-3xl font-bold tracking-[0.3em]">{created.invite_code}</code></div><div className="grid grid-cols-2 gap-2"><button type="button" onClick={copyCode} className="inline-flex h-11 items-center justify-center gap-2 rounded-xl border border-border bg-card px-4 text-xs font-semibold tracking-wide"><Copy className="h-4 w-4" /> COPIAR</button><button type="button" onClick={shareCode} className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-primary px-4 text-xs font-semibold tracking-wide text-primary-foreground"><Share2 className="h-4 w-4" /> COMPARTILHAR</button></div><button type="button" onClick={enterHub} disabled={loading} className="gradient-primary inline-flex h-12 w-full items-center justify-center gap-2 rounded-xl px-4 text-sm font-semibold text-primary-foreground disabled:opacity-60">{loading ? "ENTRANDO..." : "ENTRAR NO HUB"}<ArrowRight className="h-4 w-4" /></button>{error && <p className="text-sm text-destructive">{error}</p>}</div></div>;
