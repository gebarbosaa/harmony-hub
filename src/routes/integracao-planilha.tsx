...
function SpreadsheetIntegrationPage() {
  ...
  async function connect() {
    ...
    const name = sheetName.trim() || "Lançamentos";
    ...
    if (!url.startsWith("https://script.google.com/")) return toast.error("A URL PRECISA SER DO GOOGLE APPS SCRIPT E COMEÇAR COM https://script.google.com/");
    setLoading(true);
    try {
      const { data, error } = await supabase.functions.invoke("create-spreadsheet-connection", { body: { household_id: householdId, web_app_url: url, sheet_name: name, name: "GOOGLE SHEETS" } });
