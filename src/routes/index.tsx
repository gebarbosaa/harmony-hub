...
import { Wallet, CreditCard, Repeat, Coins, AlertTriangle, ArrowRight } from "lucide-react";
import { useEffect, useMemo, useState } from "react";
import { Area, AreaChart, CartesianGrid, ResponsiveContainer, Tooltip, XAxis, YAxis } from "recharts";
import { PageHeader, Panel, StatCard, ProgressBar, Tag } from "@/components/ui-kit";
import { formatCurrency, calculateGoalProgress, calculateBudgetUsage } from "@/lib/finance";
import { useAuth } from "@/hooks/use-auth";
import { useHouseholdTable } from "@/hooks/use-household-data";

export const Route = createFileRoute("/")({
  head: () => ({ meta: [{ title: "INÍCIO — HARMONY HUB" }, { name: "description", content: "Resumo financeiro do mês." }] }),
  component: Dashboard,
});
...
type Fixed = { id:string; name:string; category:string; amount:number; due_day:number; months:boolean[]; household_id:string };
...
type Goal = { id:string; name:string; current_amount:number; target_amount:number; monthly:number; deadline:string|null; household_id:string };
...
function Dashboard() {
  ...
  useEffect(() => { setIsMounted(true); const update=()=>setCurrentHour(new Date().getHours()); const interval=window.setInterval(update,60000); return()=>window.clearInterval(interval); },[]);
  const greeting=currentHour<12?"BOM DIA":currentHour<18?"BOA TARDE":"BOA NOITE";
  const tx=useHouseholdTable<Tx>("transactions","id,date,description,category,amount,type,paid,household_id","date");
  const fixed=useHouseholdTable<Fixed>("fixed_costs","id,name,category,amount,due_day,months,household_id");
  ...
  const goals=useHouseholdTable<Goal>("goals","id,name,current_amount,target_amount,monthly,deadline,household_id");
  ...
  const totalOutflow=expenses+installmentTotal+fixedTotal;
  ...
  return <div className="space-y-5"><PageHeader title={`${greeting}, ${profile?.name?.split(" ")[0]?.toUpperCase()||""}`} subtitle={`Veja como estão as finanças de ${monthName.toLowerCase()}.`} action={<Tag tone="primary">{monthName}</Tag>}/>
    <div className="grid grid-cols-2 gap-3 lg:grid-cols-4"><StatCard label="SALDO DO MÊS" value={formatCurrency(balance)} delta={income>0?"Receitas registradas":"Sem receitas"} tone={balance>=0?"primary":"danger"} icon={<Wallet className="h-4 w-4"/>}/><StatCard label="À VISTA" value={formatCurrency(expenses)} delta={`${currentTx.filter(t=>t.type==="DESPESA").length} despesas`} tone="danger" icon={<Coins className="h-4 w-4"/>}/><StatCard label="PARCELADOS" value={formatCurrency(installmentTotal)} delta={`${activeInstallments.length} compras ativas`} tone="info" icon={<CreditCard className="h-4 w-4"/>}/><StatCard label="CUSTOS FIXOS" value={formatCurrency(fixedTotal)} delta={`${activeFixed.length} ativos`} tone="success" icon={<Repeat className="h-4 w-4"/>}/></div>
    <Panel className="border-warning/50"><div className="flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-warning/15 text-warning"><AlertTriangle className="h-5 w-5"/></span><div><h2 className="label-caps text-sm">VENCENDO EM BREVE</h2><p className="text-xs text-muted-foreground">{upcoming.length} custos fixos nos próximos 5 dias</p></div></div><Link to="/custos-fixos" className="label-caps rounded-xl border border-primary/60 px-3 py-2 text-[11px] text-primary">VER CUSTOS</Link></div>{upcoming.length===0?<p className="mt-4 py-4 text-center text-sm text-muted-foreground">Nenhum custo fixo vencendo nos próximos 5 dias.</p>:<div className="mt-4 grid gap-2 md:grid-cols-2">{upcoming.map(b=><div key={b.id} className="flex items-center justify-between rounded-xl border border-border bg-secondary/40 px-3 py-2.5"><div><p className="label-caps text-[11px]">{b.name}</p><Tag tone="warning">DIA {b.date.getDate()}</Tag></div><div className="text-right"><p className="text-sm font-bold">{formatCurrency(b.amount)}</p><Link to="/custos-fixos" className="label-caps text-[10px] text-primary">VER CUSTO</Link></div></div>)}</div>}</Panel>
    <div className="grid gap-4 lg:grid-cols-3"><Panel title="EVOLUÇÃO RECENTE" className="lg:col-span-2"><div className="h-64">{isMounted&&<ResponsiveContainer width="100%" height="100%"><AreaChart data={monthlyEvolution}><CartesianGrid stroke="var(--border)" vertical={false}/><XAxis dataKey="mes" stroke="var(--muted-foreground)" fontSize={11} tickLine={false}/><YAxis stroke="var(--muted-foreground)" fontSize={11} tickLine={false} width={48}/><Tooltip formatter={(v:number)=>formatCurrency(v)}/><Area type="monotone" dataKey="gastos" stroke="var(--orange-primary)" fill="var(--orange-primary)" fillOpacity={0.15}/><Area type="monotone" dataKey="receitas" stroke="var(--success)" fill="transparent"/></AreaChart></ResponsiveContainer>}</div></Panel><Panel title="DISTRIBUIÇÃO DE GASTOS"><div className="h-64">{isMounted&&(categoryDistribution.length?<div className="space-y-4 pt-3">{categoryDistribution.map(cat=>{const usage=calculateBudgetUsage(cat.value,cat.budget);return <div key={cat.name} className="space-y-2"><div className="flex items-center justify-between text-sm"><span className="label-caps text-[11px]">{cat.name}</span><span className="font-semibold">{formatCurrency(cat.value)}</span></div><ProgressBar percent={usage}/></div>})}</div>:<div className="flex h-full items-center justify-center text-sm text-muted-foreground">Cadastre despesas para ver a distribuição.</div>)}</div></Panel></div>
    <div className="grid gap-4 lg:grid-cols-2"><Panel title="MAIORES CATEGORIAS"><div className="space-y-4">{categoryDistribution.length?categoryDistribution.map(cat=>{const usage=calculateBudgetUsage(cat.value,cat.budget);return <div key={cat.name} className="space-y-2"><div className="flex items-center justify-between text-sm"><span className="label-caps text-[11px]">{cat.name}</span><span className="font-semibold">{formatCurrency(cat.value)}</span></div><ProgressBar percent={usage}/><p className="text-[11px] text-muted-foreground">{Math.round(usage)}% do orçamento de {formatCurrency(cat.budget)}</p></div>}):<p className="py-8 text-center text-sm text-muted-foreground">Nenhum gasto registrado neste mês.</p>}</div></Panel><Panel title="META PRINCIPAL">{mainGoal?<><p className="label-caps text-sm">{mainGoal.name}</p><p className="mt-1 text-xs text-muted-foreground">{formatCurrency(Number(mainGoal.current_amount))} / {formatCurrency(Number(mainGoal.target_amount))}</p><div className="mt-3"><ProgressBar percent={calculateGoalProgress(Number(mainGoal.current_amount),Number(mainGoal.target_amount))} tone="primary"/></div><div className="mt-3 flex items-center justify-between text-[11px] text-muted-foreground"><span>{Math.round(calculateGoalProgress(Number(mainGoal.current_amount),Number(mainGoal.target_amount)))}% concluído</span><span>{mainGoal.deadline?`Previsão: ${mainGoal.deadline}`:"Sem prazo"}</span></div><Link to="/metas" className="label-caps mt-4 inline-flex items-center gap-1 text-[11px] text-primary">VER TODAS AS METAS <ArrowRight className="h-3 w-3"/></Link></>:<div className="py-8 text-center"><p className="text-sm text-muted-foreground">Nenhuma meta cadastrada.</p><Link to="/metas" className="label-caps mt-3 inline-flex text-[11px] text-primary">CRIAR META</Link></div>}</Panel></div>
  </div>;
}
