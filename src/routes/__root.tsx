...
const PUBLIC_ROUTES = ["/login", "/auth/callback", "/politica-de-privacidade", "/termos-de-servico"];
...
function AuthGate() {
  ...
  useEffect(() => {
    ...
    if (isPublicRoute && pathname === "/login") void navigate({ to: "/", replace: true });
  }, [loading, profileLoading, session, isPublicRoute, pathname, navigate]);
