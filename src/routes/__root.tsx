...
export const Route = createRootRouteWithContext<{ queryClient: QueryClient }>()({
  head: () => ({ meta: [{ charSet: "utf-8" }, { name: "viewport", content: "width=device-width, initial-scale=1" }, { title: "MULTICAP — GESTÃO FINANCEIRA" }, { name: "description", content: "PAINEL COMPARTILHADO DE FINANÇAS, ROTINA, METAS E HÁBITOS." }, { property: "og:type", content: "website" }, { name: "twitter:card", content: "summary_large_image" }], links: [{ rel: "stylesheet", href: appCss }, { rel: "preconnect", href: "https://fonts.googleapis.com" }, { rel: "preconnect", href: "https://fonts.gstatic.com", crossOrigin: "anonymous" }, { rel: "stylesheet", href: "https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800&display=swap" }, { rel: "icon", href: "/favicon.ico", type: "image/x-icon" }] }),
...
function RootComponent() {
  ...
  return <QueryClientProvider client={queryClient}><AuthProvider><GlobalMonthProvider><AuthGate /><Toaster position="top-center" /></GlobalMonthProvider></AuthProvider></QueryClientProvider>;
}
...
function AuthGate() {
  ...
  useEffect(() => {
    ...
    if (session && !profile?.household_id && !isOnboarding) { navigate({ to: "/onboarding" }); return; }
  ...
  if (loading) return <div className="flex min-h-screen items-center justify-center bg-background"><p className="label-caps text-sm text-muted-foreground">CARREGANDO...</p></div>;
