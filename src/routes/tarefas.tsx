...
import { Check, ChevronDown, GripVertical, ListChecks, Plus, Trash2 } from "lucide-react";
...
type QuadrantName = (typeof QUADRANTS)[number]["name"];
...
function TasksPage() {
  ...
  const [quadrant, setQuadrant] = useState<QuadrantName>("FAZER AGORA");
  const [dueDate, setDueDate] = useState("");
  const [responsible, setResponsible] = useState("AMBAS");
  const [saving, setSaving] = useState(false);
  const [draggingId, setDraggingId] = useState<string | null>(null);
  const [dragOverQuadrant, setDragOverQuadrant] = useState<QuadrantName | null>(null);
  ...
  async function moveTask(task: Task, nextQuadrant: QuadrantName) {
    if (task.quadrant === nextQuadrant) return;
    try {
      await tasks.update(task.id, { quadrant: nextQuadrant });
      toast.success(`TAREFA MOVIDA PARA ${nextQuadrant}`);
    } catch (e) { toast.error(e instanceof Error ? e.message : "ERRO AO MOVER TAREFA"); }
  }
  ...
  return <div className="space-y-5 pb-6">
    ...
    <div>
      <p className="label-caps text-[10px] text-muted-foreground">MATRIZ DE EISENHOWER</p>
      <p className="text-xs text-muted-foreground">Arraste uma tarefa para outro quadrante para alterar sua prioridade.</p>
    </div>
    {showForm && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 p-4" onMouseDown={e => { if (e.target === e.currentTarget) setShowForm(false); }}>
      <div role="dialog" aria-modal="true" aria-label="NOVA TAREFA" className="w-full max-w-3xl rounded-3xl border border-border bg-background p-5 shadow-2xl sm:p-7">
        ...
        <div className="grid gap-3 md:grid-cols-2">
          ...
          <label className="relative"><span className="pointer-events-none absolute left-4 top-2 text-[9px] text-muted-foreground">QUADRANTE</span><select value={quadrant} onChange={e => setQuadrant(e.target.value as QuadrantName)} className="h-full w-full appearance-none rounded-xl border border-input bg-background px-4 pb-2 pt-5 text-sm">{QUADRANTS.map(q => <option key={q.name}>{q.name}</option>)}</select><ChevronDown className="pointer-events-none absolute right-3 top-1/2 h-4 w-4" /></label>
    ...
    {tasks.isLoading ? <Panel><div className="py-14 text-center text-sm text-muted-foreground">CARREGANDO TAREFAS...</div></Panel> : tasks.rows.length === 0 ? <Panel><div className="py-14 text-center"><ListChecks className="mx-auto mb-3 h-10 w-10 text-primary"/><p className="font-semibold">NENHUMA TAREFA CADASTRADA</p><p className="mt-1 text-xs text-muted-foreground">Toque no + para adicionar sua primeira tarefa.</p></div></Panel> : <div className="grid gap-3 md:grid-cols-2">{QUADRANTS.map(q => { const rows = tasks.rows.filter(t => t.quadrant === q.name); return <Panel key={q.name} className={`border-2 ${q.border} transition-colors ${dragOverQuadrant === q.name ? "bg-primary/5 ring-2 ring-primary/30" : ""}`} onDragOver={(e: React.DragEvent) => { e.preventDefault(); setDragOverQuadrant(q.name); }} onDragLeave={() => setDragOverQuadrant(null)} onDrop={e => { e.preventDefault(); const task = tasks.rows.find(t => t.id === e.dataTransfer.getData("text/plain")); if (task) void moveTask(task, q.name); setDraggingId(null); setDragOverQuadrant(null); }}><div className="mb-3 flex justify-between"><div><h2 className="label-caps text-xs">{q.name}</h2><p className="text-[11px] text-muted-foreground">{q.hint}</p></div><Tag tone="neutral">{rows.length}</Tag></div>{rows.length === 0 ? <div className="rounded-xl border border-dashed border-border px-3 py-7 text-center text-[11px] text-muted-foreground">Solte uma tarefa aqui.</div> : <ul className="space-y-2">{rows.map(task => <li key={task.id} draggable onDragStart={e => { e.dataTransfer.setData("text/plain", task.id); e.dataTransfer.effectAllowed = "move"; setDraggingId(task.id); }} onDragEnd={() => { setDraggingId(null); setDragOverQuadrant(null); }} className={`flex items-center gap-3 rounded-xl border border-border bg-secondary/30 px-3 py-3 cursor-grab active:cursor-grabbing ${task.completed ? "opacity-60" : ""} ${draggingId === task.id ? "opacity-40" : ""}`}><GripVertical className="h-4 w-4 shrink-0 text-muted-foreground" aria-label="ARRASTAR"/><button onClick={() => void toggleTask(task)} className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-lg border ${task.completed ? "border-primary bg-primary text-primary-foreground" : "border-border text-transparent"}`}><Check className="h-4 w-4"/></button><div className="min-w-0 flex-1"><p className={`label-caps truncate text-[11px] ${task.completed ? "line-through" : ""}`}>{task.title}</p><PersonDot name={task.responsible || "AMBAS"}/></div><Tag tone={task.due_date === new Date().toISOString().slice(0,10) ? "danger" : "neutral"}>{formatDueDate(task.due_date)}</Tag><button onClick={() => void removeTask(task.id)} className="text-red-400"><Trash2 className="h-4 w-4"/></button></li>)}</ul>}</Panel>; })}</div>}
  </div>;
}
