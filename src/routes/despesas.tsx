...
export const Route = createFileRoute("/despesas")({
  head: () => ({ meta: [{ title: "DESPESAS — MULTICAP" }] }),
...
function DespesasPage() {
  const { month, setMonth } = useGlobalMonth("despesas");
  ...
  const visible = useMemo(() => tx.rows.filter(t => t.type === "DESPESA" && t.date.startsWith(month) && t.description.toLowerCase().includes(search.toLowerCase())), [tx.rows, month, search]);
  ...
  async function add() {
    ...
    try {
      await tx.insert({ date, description: description.trim().toUpperCase(), amount: parsed, type: "DESPESA", category, pay_method: payMethod, responsible, paid: true });
      setDescription(""); setAmount(""); toast.success("DESPESA SALVA");
  ...
  async function removeRow(t: Row) {
    ...
    try { await tx.remove(t.id); toast.success("DESPESA EXCLUÍDA"); } catch (e) { toast.error(e instanceof Error ? e.message : "ERRO AO EXCLUIR"); }
  }
  return <div className="space-y-5">
    <PageHeader title="DESPESAS" subtitle={`SAÍDAS REGISTRADAS NO MÊS ${month}.`} action={<MonthSelector month={month} setMonth={setMonth} />} />
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2"><StatCard label="TOTAL DE DESPESAS" value={formatCurrency(total)} tone="danger" /><StatCard label="LANÇAMENTOS" value={String(visible.length)} tone="primary" /></div>
    <Panel title="NOVA DESPESA"><div className="grid gap-3 md:grid-cols-6">
      <label className="md:col-span-2"><span className="label-caps text-[10px] text-muted-foreground">DESCRIÇÃO</span><input value={description} onChange={e => setDescription(e.target.value)} placeholder="MERCADO, ALUGUEL, UBER..." className="mt-1 w-full rounded-xl border border-input bg-background px-3 py-2.5 text-sm" /></label>
      ...
      <button type="button" onClick={add} className="gradient-primary label-caps rounded-xl px-3 py-2 text-[10px] text-primary-foreground md:col-span-2">SALVAR DESPESA</button>
    </div></Panel>
    <Panel title={`DESPESAS — ${month}`} aside={<input value={search} onChange={e => setSearch(e.target.value)} placeholder="BUSCAR..." className="rounded-lg border border-input bg-background px-2 py-1.5 text-xs" />}>
      {tx.isLoading ? <p className="py-8 text-center text-sm text-muted-foreground">CARREGANDO...</p> : visible.length === 0 ? <p className="py-8 text-center text-sm text-muted-foreground">NENHUMA DESPESA CADASTRADA NESTE MÊS.</p> : <ul className="divide-y divide-border">{visible.map(t => <li key={t.id} className="flex items-center justify-between gap-3 py-3"><div className="min-w-0"><p className="label-caps truncate text-[11px]">{t.description}</p><div className="mt-1 flex flex-wrap items-center gap-2"><Tag>{t.category}</Tag><PersonDot name={t.responsible} /><span className="text-[10px] text-muted-foreground">{t.pay_method}</span><span className="text-[10px] text-muted-foreground">{t.date}</span></div></div><div className="flex items-center gap-3"><p className="text-sm font-bold text-danger">-{formatCurrency(Number(t.amount))}</p><button type="button" onClick={() => void removeRow(t)} className="text-xs text-danger">EXCLUIR</button></div></li>)}</ul>}
    </Panel></div>;
}
