...
function ContasAPagarPage() {
  ...
  async function addPayable() {
    ...
    if (!category) { toast.error("CADASTRE UMA CATEGORIA EM AJUSTES"); return; }
    try {
      await insert({ description: description.trim(), amount: value, due_date: dueDate, category, account_id: null, payment_method_id: null, card_id: null, responsible: "", status: "PENDENTE", linked_transaction_id: null });
