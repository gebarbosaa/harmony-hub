...
function ContasAPagarPage() {
  ...
  async function markPaid(item: Payable) {
    try {
      ...
      if (!transactionId) {
        const created = await transactions.insert({ date: item.due_date, description: item.description, amount: Number(item.amount), type: "DESPESA", category: item.category || "OUTROS", pay_method: "PIX", payment_method_id: item.payment_method_id ?? null, payment_method_name: null, card_id: item.card_id ?? null, card_name: null, account_id: item.account_id ?? null, responsible: item.responsible || "AMBAS", paid: true, source_type: "accounts_payable", source_id: item.id });
        transactionId = created.id;
      }
      await update(item.id, { status: "PAGA", paid_at: new Date().toISOString(), linked_transaction_id: transactionId });
