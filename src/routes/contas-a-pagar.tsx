...
type Payable = { id: string; household_id: string; description: string; amount: number; due_date: string; category: string; account_id: string | null; payment_method_id: string | null; card_id: string | null; responsible: string; status: "PENDENTE" | "PAGA" | "CANCELADA"; recurrence?: string | null; notes?: string | null; paid_at?: string | null; linked_transaction_id?: string | null };
...
function ContasAPagarPage() {
  ...
  function openPayment(item: Payable) { setPayableToPay(item); setPaidAccountId(item.account_id ?? accounts.rows[0]?.id ?? ""); setPaidMethod(item.payment_method_id ? "" : ""); setPaidResponsible(item.responsible || "AMBAS"); setPayOpen(true); }
  function closePayment() { setPayOpen(false); setPayableToPay(null); setPaidAccountId(""); setPaidMethod(""); setPaidResponsible("AMBAS"); }
  ...
  async function markPaid(item: Payable, accountId: string, method: string, responsible: string) {
    if (!accountId || !method || !responsible) { toast.error("PREENCHA CONTA, FORMA DE PAGAMENTO E QUEM PAGOU"); return; }
    try {
      ...
      if (!transactionId) {
        const created = await transactions.insert({ date: new Date().toISOString().slice(0, 10), description: item.description, amount: Number(item.amount), type: "DESPESA", category: item.category || "OUTROS", pay_method: method, payment_method_id: null, payment_method_name: method, card_id: null, card_name: null, account_id: accountId, responsible, paid: true, source_type: "accounts_payable", source_id: item.id });
        transactionId = created.id;
      } else {
        await transactions.update(transactionId, { date: new Date().toISOString().slice(0, 10), pay_method: method, payment_method_id: null, payment_method_name: method, account_id: accountId, responsible, paid: true });
      }
      await update(item.id, { status: "PAGA", paid_at: new Date().toISOString(), account_id: accountId, responsible, linked_transaction_id: transactionId });
      closePayment();
