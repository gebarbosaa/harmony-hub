...
const RECURRENCE_OPTIONS = ["NÃO RECORRENTE", "MENSAL", "BIMESTRAL", "TRIMESTRAL", "SEMESTRAL", "ANUAL"];
...
function ContasAPagarPage() {
  ...
  function resetForm() { setDescription(""); setAmount(""); setDueDate(""); setCategory("Outros"); setRecurrence("NÃO RECORRENTE"); }
  ...
  async function addPayable() {
    ...
    try {
      await insert({ description: description.trim(), amount: value, due_date: dueDate, category: category.trim() || "Outros", account_id: null, payment_method_id: null, card_id: null, responsible: "", status: "PENDENTE", recurrence: recurrence === "NÃO RECORRENTE" ? null : recurrence, linked_transaction_id: null });
  ...
  async function markPaid(item: Payable, accountId: string, method: string, cardId: string, responsible: string) {
    ...
    try {
      ...
      if (!transactionId) {
        const created = await transactions.insert({ ...transactionData, source_type: "accounts_payable", source_id: item.id });
