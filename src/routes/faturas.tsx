...
function InvoicesPage(){
 ...
 const[selected,setSelected]=useState<Card|null>(null);const[editing,setEditing]=useState<Card|null>(null);
 ...
 <Panel title={`FATURAS — ${month}`}>{cards.rows.length===0?<p className="py-8 text-center text-sm text-muted-foreground">NENHUM CARTÃO CADASTRADO.</p>:<div className="space-y-2">{cards.rows.map(c=>{const total=cardTotal(c);const inv=monthInvoices.find(i=>i.card_id===c.id);return <div key={c.id} className="flex items-center justify-between rounded-xl border border-border bg-secondary/30 px-4 py-3"><div><p className="label-caps text-[11px]">{c.name}</p><p className="text-[10px] text-muted-foreground">FATURA {month} · FECHA DIA {c.close_day} · VENCE DIA {c.due_day}</p></div><div className="flex items-center gap-3"><b>{formatCurrency(total)}</b>{inv&&<button onClick={()=>void toggleInvoice(inv)}><Tag tone={inv.status==="PAGA"?"success":"warning"}>{inv.status}</Tag></button>}</div></div>})}</div>}</Panel>
