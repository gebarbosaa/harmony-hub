...
function MarketPage() {
  ...
  async function addItem() {
    ...
    try {
      ...
      setName(""); setListName(""); setPrice(""); setQty("1"); setShowForm(false); toast.success("ITEM ADICIONADO");
  ...
  return <div className="space-y-5 pb-8">
    ...
    {showForm && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onMouseDown={e => { if (e.currentTarget === e.target) setShowForm(false); }}>
      <div className="w-full max-w-lg rounded-2xl border border-border bg-background p-5 shadow-xl">
        ...
        <div className="grid gap-3 md:grid-cols-2">
          {!current && <input autoFocus value={listName} onChange={e => setListName(e.target.value)} placeholder="NOME DA LISTA (EX.: SEMANA)" className="rounded-xl border border-input bg-background px-3 py-3 text-sm md:col-span-2" />}
          <input autoFocus={Boolean(current)} value={name} onChange={e => setName(e.target.value)} placeholder="PRODUTO" className="rounded-xl border border-input bg-background px-3 py-3 text-sm md:col-span-2"/>
          <input value={qty} onChange={e => setQty(e.target.value)} type="number" min="1" placeholder="QUANTIDADE" className="rounded-xl border border-input bg-background px-3 py-3 text-sm"/>
          <input value={price} onChange={e => setPrice(e.target.value)} placeholder="PREÇO" inputMode="decimal" className="rounded-xl border border-input bg-background px-3 py-3 text-sm"/>
          <button onClick={() => void addItem()} className="gradient-primary rounded-xl px-4 py-3 text-[11px] text-primary-foreground md:col-span-2">ADICIONAR</button>
        </div>
      </div>
    </div>}
