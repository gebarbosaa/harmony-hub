...
function MarketPage() {
  ...
  return <div className="space-y-5 pb-8">
    ...
    <div className="flex w-fit gap-1 rounded-xl border border-border bg-secondary/30 p-1"><button onClick={() => setTab("mercado")} className={cn("label-caps rounded-lg px-4 py-2 text-[11px] transition", tab === "mercado" ? "bg-background text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground")}>MERCADO</button><button onClick={() => setTab("modo")} className={cn("label-caps rounded-lg px-4 py-2 text-[11px] transition", tab === "modo" ? "bg-background text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground")}>MODO MERCADO</button></div>
    {tab === "mercado" ? <><div className="grid grid-cols-2 gap-3 lg:grid-cols-3"><StatCard label="LISTAS" value={String(lists.rows.length)} tone="info"/><StatCard label="ITENS" value={String(totalItems)} tone="primary"/><StatCard label="CONCLUÍDOS" value={String(doneItems)} tone="success"/></div><Panel title="SUAS LISTAS DE COMPRAS"><div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">{lists.rows.map(list => { const count = items.rows.filter(i => i.list_id === list.id).length; const done = items.rows.filter(i => i.list_id === list.id && i.done).length; return <button key={list.id} onClick={() => { setActive(list.id); setTab("modo"); }} className="rounded-2xl border border-border p-4 text-left transition hover:border-primary/50 hover:bg-secondary/30"><p className="label-caps text-sm">{list.name}</p><p className="mt-2 text-xs text-muted-foreground">{count} ITENS · {done} CONCLUÍDOS</p></button>; })}{lists.rows.length === 0 && <div className="md:col-span-2 lg:col-span-3 py-10 text-center text-sm text-muted-foreground">NENHUMA LISTA CADASTRADA. ENTRE EM MODO MERCADO E CLIQUE NO + PARA CRIAR.</div>}</div></Panel><Panel title="RESUMO DE COMPRAS"><p className="text-xs text-muted-foreground">TOTAL ESTIMADO DAS LISTAS ATIVAS</p><p className="mt-1 text-2xl font-semibold">{formatCurrency(totalEstimated)}</p></Panel></> : <>
