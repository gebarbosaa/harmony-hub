...
function MarketPage() {
  ...
  return <div className="space-y-5 pb-8">
    ...
    {tab === "mercado" ? <>
      ...
      <Panel title="SUAS LISTAS DE COMPRAS"><div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">{lists.rows.map(list => { const count = items.rows.filter(i => i.list_id === list.id).length; const done = items.rows.filter(i => i.list_id === list.id && i.done).length; return <button key={list.id} onClick={() => { setActive(list.id); setTab("modo"); }} className="rounded-2xl border border-border p-4 text-left transition hover:border-primary/50 hover:bg-secondary/30"><p className="label-caps text-sm">{list.name}</p><p className="mt-2 text-xs text-muted-foreground">{count} ITENS · {done} CONCLUÍDOS</p></button>; })}{lists.rows.length === 0 && <div className="md:col-span-2 lg:col-span-3 py-10 text-center text-sm text-muted-foreground">NENHUMA LISTA CADASTRADA. CLIQUE NO + PARA CRIAR.</div>}</div></Panel>
