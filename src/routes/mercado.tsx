...
function MarketPage(){
 ...
 async function addItem(){
   ...
   try{
     ...
     await items.insert({list_id:target.id,name:product.toUpperCase(),price:parsedPrice,qty:parsedQty,unit:"UN",category:"OUTROS",priority:"MÉDIA",done:false});
     setShowForm(false);
     setName("");setListName("");setPrice("");setQty("");
     setTab("modo");
 ...
 return <div className="space-y-5 pb-8">
  ...
  <><div className="flex flex-wrap gap-2">{lists.rows.map(l=><button type="button" key={l.id} onClick={()=>setActive(l.id)} className={cn("label-caps rounded-xl border px-4 py-2 text-[11px]",current?.id===l.id?"border-primary bg-primary/15 text-primary":"border-border text-muted-foreground")}>{l.name}</button>)}</div>{current?<><div className="grid grid-cols-2 gap-3 lg:grid-cols-3"><StatCard label="ITENS" value={String(currentItems.length)} tone="info"/><StatCard label="CONCLUÍDOS" value={String(currentItems.filter(i=>i.done).length)} tone="success"/><StatCard label="TOTAL ESTIMADO" value={formatCurrency(estimated)} tone="primary"/></div><Panel title="ITENS DO CARRINHO"><ul className="space-y-3">{currentItems.map(i=><li key={i.id} className={cn("rounded-2xl border border-border bg-secondary/30 p-3",i.done&&"border-primary/50")}><div className="flex items-center gap-3"><button type="button" onClick={()=>void toggle(i)} className={cn("flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border",i.done?"gradient-primary border-transparent text-primary-foreground":"border-border text-transparent")}><Check className="h-5 w-5"/></button><div className="min-w-0 flex-1"><p className={cn("label-caps text-sm",i.done&&"line-through opacity-60")}>{i.name}</p><div className="mt-1 flex gap-2"><Tag>{i.qty==null?"QUANTIDADE NÃO DEFINIDA":`${i.qty} ${i.unit}`}</Tag><Tag>{i.priority}</Tag></div></div><span className="text-sm font-semibold">{formatCurrency((Number(i.qty)||0)*(Number(i.price)||0))}</span></div><div className="mt-3 flex items-center justify-between"><div className="flex items-center gap-2"><button type="button" onClick={()=>void changeQty(i,-1)} className="flex h-9 w-9 items-center justify-center rounded-lg border border-border"><Minus className="h-4 w-4"/></button><span className="w-8 text-center text-sm font-semibold">{i.qty??"—"}</span><button type="button" onClick={()=>void changeQty(i,1)} className="flex h-9 w-9 items-center justify-center rounded-lg border border-border"><Plus className="h-4 w-4"/></button></div><span className="text-xs text-muted-foreground">{i.price==null?"SEM PREÇO":`${formatCurrency(Number(i.price))} / ${i.unit}`}</span></div></li>)}{!currentItems.length&&<li className="py-10 text-center text-sm text-muted-foreground">NENHUM ITEM CADASTRADO NESTA LISTA.</li>}</ul></Panel></>:<Panel title="LISTA DE COMPRAS"><div className="py-8 text-center text-sm text-muted-foreground">NENHUMA LISTA CADASTRADA. USE O + PARA CRIAR SUA LISTA E ADICIONAR O PRIMEIRO ITEM.</div></Panel>}</>}
 </div>;
}
