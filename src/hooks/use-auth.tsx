...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  function setProfileSafe(next: Profile | null) { profileRef.current = next; setProfile(next); }
  ...
  function setActiveHousehold(householdId: string) {
    ...
    if (!current || current.household_id === householdId) return;
    setProfileSafe({ ...current, household_id: householdId });
  ...
  useEffect(() => {
    ...
    const applySession = async (nextSession: Session | null) => {
      ...
      if (!nextSession) { ++profileRequestRef.current; setProfileSafe(null); setProfileLoading(false); setLoading(false); return; }
      setProfileLoading(true);
